(() => {
  ...
  style.textContent = `
    ...
    .ham-starts-title { margin-bottom: 6px; color: #f4f7fb; font-size: 12px; font-weight: 800; }
    .ham-starts-list { color: #b8c7d8; font-size: 12px; line-height: 1.55; }
  ...
  function card(band, date) {
    ...
    return `
      <article class="signal-card" data-ham-card="true">
        <div class="card-top">
          <div>
            <div class="ham-range">${band.minMHz.toFixed(band.minMHz < 10 ? 3 : 3)}–${band.maxMHz.toFixed(band.maxMHz < 10 ? 3 : 3)}<span>MHz</span></div>
        ...
        <div class="ham-starts">
          <div class="ham-starts-title">Good places to start tuning</div>
          <div class="ham-starts-list">${band.starts.join(' · ')}</div>
        </div>
  ...
  function renderHam() {
    ...
    grid.innerHTML = `
      <div class="ham-disclaimer">Amateur radio is not scheduled like international broadcasting. These cards show U.S./ITU Region 2 HF amateur allocations and likely band activity for the selected time. Exact activity changes minute to minute.</div>
