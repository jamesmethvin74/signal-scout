...
### Taiwan — regulator-grade MW
- Authority: National Communications Commission (NCC), Taiwan
- Official source: `AM radio broadcast frequency and transmitter location` / `無線調幅AM廣播電臺頻率及發射機位址紀錄`
- Official build input: NCC ODS published from the government open-data catalog
- Source date used by this milestone: 7 April 2026
- Licence: Open Government Data License, version 1.0; free government open data
- Fields ingested: AM station name, frequency kHz, transmitter address, east longitude, north latitude, station class/category
- Source basis: NCC states the table is based on issued station licences
- Coordinate handling: published transmitter-site longitude/latitude is used directly; no city, county, or country centroid is substituted
- Frequency handling: only 520–1710 kHz is admitted. The NCC document also contains higher-frequency overseas/shortwave records, and those are rejected from the MW catalog.
- Power handling: this NCC dataset does not publish transmitter power. FREQBEACON does not infer or fabricate one; these records therefore contribute regulator-grade station/site identity and exact geographic distance without a synthetic power value.
- Runtime: the ODS is fetched and normalized only during the build; the app does not query NCC while listening
- Confidence: Tier 1 / regulator for licensed station identity, frequency, transmitter site, and coordinates
- Refresh: rerun the generator against the current official NCC document; fail closed on missing schema, implausible Taiwan coordinates, suspicious record count, or loss of the 531 kHz current marker
...
- `freqbeacon-zero-global-mw-lw.js` — regulator-grade Canada/UK/Australia/Taiwan records
