// FREQBEACON Zero qualification bench — automatic qualification v3.
// Calibrates each receiver's actually delivered FAST waterfall cadence, then grades
// band/mode retunes relative to that measured baseline. Test harness only: no /zero,
// renderer, socket topology or Kiwi protocol implementation changes.
...
const CALIBRATION_SETTLE_MS = 5000;
...
function snap() {
  ...
  return {
    ...
    ackTarget: parts.length > 1 ? numberFrom(parts[1]) : NaN,
...
panel.innerHTML = `
  <div class="bench-auto-head">
    <div><small>AUTO QUALIFICATION V3</small><strong>Calibrate real FAST cadence → retune → compare</strong></div>
    <button type="button" data-auto-v3-run>RUN AUTO TEST</button>
  </div>
  <div class="bench-auto-status" data-auto-v3-status>Measures this receiver's real delivered FAST cadence first. Retunes are graded against that measured baseline, not the theoretical 23-FPS request.</div>
  <div class="bench-auto-results" data-auto-v3-results hidden></div>
`;
...
style.textContent = `
  ...
  .bench-auto-results .auto-row { display:grid; grid-template-columns:minmax(108px,1.05fr) .82fr .72fr .82fr .65fr; gap:6px; padding:6px 0; border-top:1px solid rgba(94,126,136,.18); }
...
const runButton = panel.querySelector('[data-auto-v3-run]');
const statusEl = panel.querySelector('[data-auto-v3-status]');
const resultsEl = panel.querySelector('[data-auto-v3-results]');
let running = false;
...
async function calibrateBaseline() {
  ...
  statusEl.textContent = `Calibrating delivered FAST cadence · settling ${Math.round(CALIBRATION_SETTLE_MS/1000)}s · ${loadText(loadStart)}`;
  await sleep(CALIBRATION_SETTLE_MS);

  const samples = [];
  for (let i = 0; i < CALIBRATION_SAMPLES; i += 1) {
    const s = snap();
    if (!stablePair(s)) throw new Error('Socket pair changed during baseline calibration');
    samples.push(s);
    statusEl.textContent = `Calibrating delivered FAST cadence · sample ${i+1}/${CALIBRATION_SAMPLES} · ${fmt(s.wf)} FPS · ACK ${fmt(s.ackTarget,0)}`;
    await sleep(1000);
  }

  const wf = samples.map((s) => s.wf);
  const baselineAvg = avg(wf);
  const baselineMin = min(wf);
  const baselineMax = max(wf);
  const ackTarget = samples.map((s) => s.ackTarget).find(Number.isFinite) ?? NaN;
  const loadReady = await loadSnapshot();
  const healthy = Number.isFinite(baselineAvg) && baselineAvg >= MIN_BASELINE_FPS && stablePair();
  return { healthy, baselineAvg, baselineMin, baselineMax, ackTarget, loadStart, loadReady };
}

async function waitForRecovery(stageLabel, baselineFps) {
  const started = performance.now();
  const deadline = started + STAGE_RECOVERY_TIMEOUT_MS;
  let good = 0;
  let best = 0;
  const threshold = baselineFps * RECOVERY_RATIO;
  while (performance.now() < deadline) {
    const s = snap();
    ...
    good = Number.isFinite(s.wf) && s.wf >= threshold ? good + 1 : 0;
    statusEl.textContent = `${stageLabel}: recovery ${fmt(s.wf)} FPS · need ${fmt(threshold)} · ${good}/${STAGE_GOOD_SAMPLES}`;
...
async function runStage(stage, index, baselineFps) {
  ...
  const recovery = await waitForRecovery(stage.label, baselineFps);
  const samples = [];
  for (let i = 0; i < MEASURE_SAMPLES; i += 1) {
    const s = snap();
    samples.push(s);
    if (!stablePair(s)) break;
    statusEl.textContent = `Stage ${index+1}/${TEST_STAGES.length}: ${stage.label} · sample ${i+1}/${MEASURE_SAMPLES} · W/F ${fmt(s.wf)} · baseline ${fmt(baselineFps)} · SND ${fmt(s.snd)}`;
    await sleep(1000);
  }
  ...
  return {
    ...
    ratioToBaseline: avg(wf) / baselineFps,
...
function classify(r) {
  ...
  if (!r.recovered) return r.ratioToBaseline >= WARN_RATIO ? 'warn' : 'fail';
  if (r.ratioToBaseline >= PASS_RATIO && r.recoverySec <= 10) return 'pass';
  if (r.ratioToBaseline >= WARN_RATIO) return 'warn';
  return 'fail';
}

function renderInconclusive(baseline) {
  resultsEl.hidden = false;
  resultsEl.innerHTML = `<div class="auto-summary inconclusive">INCONCLUSIVE — delivered FAST baseline too low to qualify retunes · avg ${fmt(baseline.baselineAvg)} FPS · min/max ${fmt(baseline.baselineMin)}/${fmt(baseline.baselineMax)} · ACK ${fmt(baseline.ackTarget,0)} · load ${loadText(baseline.loadStart)} → ${loadText(baseline.loadReady)} · pair ${stablePair() ? 'STABLE' : 'CHANGED'}</div>`;
  statusEl.textContent = 'INCONCLUSIVE: receiver cannot provide a stable minimum cadence for an engine qualification run.';
}
...
function renderResults(results, baseline) {
  ...
  const rows = results.map((r) => {
    ...
    return `<div class="auto-row ${level}"><b>${r.label}</b><span>WF ${fmt(r.wfAvg)} (${fmt(r.ratioToBaseline*100,0)}%)</span><span>REC ${fmt(r.recoverySec)}s</span><span>LOAD ${load}</span><span>${level.toUpperCase()}</span></div>`;
  }).join('');
  ...
  const title = overall === 'pass' ? 'PASS — band/mode engine qualified against measured receiver baseline' : overall === 'warn' ? 'PASS WITH RECEIVER-CADENCE WARNINGS' : 'FAIL — retune/session degradation exceeds calibrated baseline tolerance';
  const overallWf = avg(results.map((r) => r.wfAvg));
  resultsEl.innerHTML = `${rows}<div class="auto-summary ${overall}">${title} · calibrated FAST ${fmt(baseline.baselineAvg)} FPS (ACK ${fmt(baseline.ackTarget,0)}) · average after retunes ${fmt(overallWf)} FPS · pair ${pairOk ? 'STABLE' : 'CHANGED'} · load ${loadText(baseline.loadStart)} → ${loadText(baseline.loadReady)}</div>`;
...
runButton?.addEventListener('click', async () => {
  ...
  resultsEl.innerHTML = '';
  try {
    statusEl.textContent = 'Starting calibrated qualification bench…';
    ...
    for (let i = 0; i < TEST_STAGES.length; i += 1) {
      const result = await runStage(TEST_STAGES[i], i, baseline.baselineAvg);
      results.push(result);
