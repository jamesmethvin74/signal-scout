...
const ISED_URLS = [
  'https://sms-sgs.ic.gc.ca/eic/site/sms-sgs-prod.nsf/vwapj/baserad.zip/$FILE/baserad.zip',
  'https://spectrum.ic.gc.ca/engineering/engdoc/baserad.zip'
];
...
const UTILITY_RE = /\b(?:ndb|beacon|navtex|navigation|aero|aviation|marine|maritime|coast guard|weather fax|fax|rtty|time signal|standard frequency)\b/i;

async function fetchBuffer(url,label){ const r=await fetch(url,{redirect:'follow',headers:{'user-agent':'FREQBEACON catalog builder/2.0'},signal:AbortSignal.timeout(60000)}); if(!r.ok) throw new Error(`${label} fetch failed: ${r.status} ${r.statusText}`); return Buffer.from(await r.arrayBuffer()); }
async function fetchText(url,label){ return (await fetchBuffer(url,label)).toString('utf8').replace(/^\uFEFF/,''); }
async function fetchFirst(urls,label){ let last; for(const u of urls){try{return {url:u,buffer:await fetchBuffer(u,label)};}catch(e){last=e;}} throw last||new Error(`${label} unavailable`); }
...
export function normalizeISED(dbfBuffer){
  ...
  for(const r of records){
    ...
    if(!validCoord(nightLat,nightLon)) continue; const dayLat=validCoord(dlat,dlon)?dlat:nightLat, dayLon=validCoord(dlat,dlon)?dlon:nightLon;
...
export function normalizeOfcom(csvText){
  ...
  for(const r of objects){
    ...
    const ngr=pick(r,['OS National Grid Reference','OS NGR','NGR']); let coord; try{coord=osGridToWgs84(ngr);}catch{continue;} const powerW=ofcomPower(r); if(!Number.isFinite(powerW)||powerW<=0) continue;
    const name=pick(r,['Station']), area=pick(r,['Area']), site=pick(r,['Site']), date=pick(r,['Date','Effective Date','Change Date']);
    out.push({...stationBase('Ofcom',1,'United Kingdom'),id:`ofcom:${frequencyKHz}:${headerKey(name)}:${headerKey(site)}`,sourceId:`${frequencyKHz}:${name}:${site}`,band:'MW',frequencyKHz,callsign:'',name,location:[site,area].filter(Boolean).join(', '),country:'United Kingdom',serviceArea:area,lat:coord.lat,lon:coord.lon,powerW,mode:'AM',status:'On air',categories:['broadcast','MW'],description:`Ofcom on-air MF transmitter serving ${area||'the United Kingdom'}.`,source:'Ofcom Technical parameters for broadcast radio transmitters — MF CSV',sourceDate:date||undefined,locationApproximate:false});
  } return out;
}

function coordinateValue(v,isLon=false){
  const raw=clean(v); if(!raw) return null; const simple=Number(raw); if(Number.isFinite(simple)&&Math.abs(simple)<=(isLon?180:90)) return simple;
  const parts=raw.match(/-?\d+(?:\.\d+)?/g)?.map(Number)||[]; if(parts.length>=3){const sign=/[SW]/i.test(raw)||parts[0]<0?-1:1; return sign*(Math.abs(parts[0])+parts[1]/60+parts[2]/3600);} return null;
}
function findHeaderRow(rows){ for(let i=0;i<Math.min(rows.length,25);i++){const keys=rows[i].map(headerKey); if(keys.some(k=>k.includes('callsign'))&&keys.some(k=>k.includes('frequency'))) return i;} return -1; }
export function normalizeACMARows(rows){
  const hi=findHeaderRow(rows); if(hi<0) throw new Error('ACMA AM sheet header row not found'); const headers=rows[hi].map(clean); requireAnyHeaders(headers,[['Callsign','Call Sign'],['Frequency','Frequency (kHz)','Frequency kHz'],['Service Area','Area Served'],['Maximum ERP (W)','Max ERP (W)','Power (W)']], 'ACMA AM sheet'); const objects=rowsToObjects([headers,...rows.slice(hi+1)]), out=[];
  for(const r of objects){
    const callsign=pick(r,['Callsign','Call Sign']).toUpperCase(), frequencyKHz=num(pick(r,['Frequency (kHz)','Frequency kHz','Frequency'])); if(!callsign||!Number.isFinite(frequencyKHz)||frequencyKHz<500||frequencyKHz>1800) continue;
    ...
    const powerHeader=Object.keys(r).find(k=>/maximum.*erp.*\(w\)|max.*erp.*\(w\)|power.*\(w\)/i.test(k)); if(!powerHeader) throw new Error(`ACMA AM power header lacks explicit watts unit; got ${Object.keys(r).join(', ')}`); const powerW=num(r[powerHeader]); if(!Number.isFinite(powerW)||powerW<=0) continue;
    const area=pick(r,['Service Area','Area Served']), site=pick(r,['Transmitter','Transmitter Site','Site','Location']), purpose=pick(r,['Purpose','Service Type']), licence=pick(r,['Licence Number','License Number','Licence No']);
    out.push({...stationBase('ACMA',1,'Australia'),id:`acma:${licence||callsign}:${frequencyKHz}:${headerKey(site)}`,sourceId:licence||`${callsign}:${frequencyKHz}:${site}`,band:'MW',frequencyKHz,callsign,name:callsign,location:[site,area].filter(Boolean).join(', '),country:'Australia',serviceArea:area,lat,lon,powerW,mode:'AM',status:'Licensed',class:purpose||undefined,categories:['broadcast','MW'],description:`ACMA licensed AM transmitter${area?` serving ${area}`:''}.`,source:'Australian Communications and Media Authority Licensed Broadcasting Transmitter Data',locationApproximate:false});
  } return out;
}
export function normalizeACMA(outerZip){
  const outer=unzipEntries(outerZip); const xlsx=[...outer.entries()].find(([n])=>/\.xlsx$/i.test(n)); if(!xlsx) throw new Error('ACMA ZIP contains no XLSX'); const sheets=parseXlsxSheets(xlsx[1]); const sheet=sheets.find(s=>/^am$/i.test(clean(s.name)))||sheets.find(s=>/\bam\b|mf/i.test(s.name)); if(!sheet) throw new Error(`ACMA workbook missing AM/MF sheet; sheets: ${sheets.map(s=>s.name).join(', ')}`);
...
export function normalizeLowFrequencyFallback(scheduleText,countryText){
  const rows=parseCsv(scheduleText); const objects=rowsToObjects(rows), countries=countryCentroids(countryText), out=[];
  for(const r of objects){
    if(clean(r.Source)!=='EiBi') continue; const hz=num(r.Frequency), kHz=hz/1000; const inLW=kHz>=148.5&&kHz<=283.5, inMW=kHz>=520&&kHz<=1710; if(!inLW&&!inMW) continue;
    ...
    const country=clean(r['TX Country']||r.Origin)||'Unknown', c=countries.get(countryKey(country)); const band=inLW?'LW':'MW';
    out.push({...stationBase('EiBi','reference/fallback',country),id:`eibi:${Math.round(hz)}:${headerKey(station)}:${clean(r.On)}:${clean(r.Off)}`,sourceId:`${Math.round(hz)}:${station}:${clean(r.On)}-${clean(r.Off)}`,band,frequencyKHz:kHz,name:station,location:clean(r.Site)||country,country, ...(c?{lat:c.lat,lon:c.lon,locationApproximate:true}: {locationApproximate:true}),powerW:(()=>{const kw=num(r.Power);return Number.isFinite(kw)&&kw>0?kw*1000:undefined;})(),mode:clean(r.M)||clean(r.Mode)||'AM',language,categories:['broadcast',band],description:`EiBi reference broadcast schedule${clean(r.Target)?`. Target: ${clean(r.Target)}`:''}.`,start:clean(r.On).padStart(4,'0'),end:clean(r.Off).padStart(4,'0'),days:clean(r.Days)||'1234567',target:clean(r.Target),source:'EiBi via FREQBEACON A26 merged schedule',season:'A26',sourceCommit:A26_COMMIT});
  } return out;
}

function validate(name,entries,min){ if(entries.length<min) throw new Error(`Refusing suspicious ${name} catalog: ${entries.length} records (<${min})`); for(const e of entries){if(!Number.isFinite(e.frequencyKHz)||e.frequencyKHz<=0)throw new Error(`${name}: invalid frequency`); if(Number.isFinite(e.lat)&&!validCoord(e.lat,e.lon))throw new Error(`${name}: invalid coordinates`); const p=[e.powerW,e.dayPowerW,e.nightPowerW,e.criticalPowerW].filter(Number.isFinite); if(p.some(v=>v<0||v>5e6))throw new Error(`${name}: implausible power`);} }
...
function render(entries,meta,varName,metaName,comment){ const raw=JSON.stringify(entries); return `(() => {\n  'use strict';\n  // ${comment} Do not hand-edit.\n  const entries = ${raw};\n  window.${varName} = Object.freeze(entries.map((e) => Object.freeze({...e, categories:Object.freeze([...(e.categories||[])])})));\n  window.${metaName} = Object.freeze(${JSON.stringify(meta)});\n})();\n`; }

async function main(){
  const [isedSource,ofcomText,acmaZip,a26Text,countryText]=await Promise.all([fetchFirst(ISED_URLS,'ISED broadcasting database'),fetchText(OFCOM_URL,'Ofcom MF CSV'),fetchBuffer(ACMA_URL,'ACMA transmitter workbook'),fetchText(A26_URL,'A26 merged schedule'),fetchText(COUNTRY_URL,'country centroids')]);
  const isedZip=unzipEntries(isedSource.buffer), amDbf=findZipEntry(isedZip,'amstatio.dbf'); if(!amDbf) throw new Error('ISED archive missing AMSTATIO.DBF');
  ...
  console.log(`FREQBEACON terrestrial catalogs: ${regulatorEntries.length} regulator records (${Buffer.byteLength(regulatorJs)} B JS), ${fallbackEntries.length} fallback records (${Buffer.byteLength(fallbackJs)} B JS).`);
}
