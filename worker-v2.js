...
async function patchSdrPlayerRuntime(response) {
  ...
  const newLiveClose = `    socket.onclose = () => {\n      if (!sdr.manualStop && !sdr.gotAudio) failCurrentReceiver('Receiver did not answer. Trying the next ranked receiver…');\n      else if (!sdr.manualStop && sdr.gotAudio) {\n        const liveMs = sdr.liveStartedAt ? Date.now() - sdr.liveStartedAt : 0;\n        if (liveMs > 0 && liveMs < SHORT_LIVE_RETRY_MS && Number(sdr.shortDisconnectRetries || 0) < 1) {\n          const retryIndex = sdr.receiverIndex;\n          sdr.shortDisconnectRetries = Number(sdr.shortDisconnectRetries || 0) + 1;\n          window.clearTimeout(sdr.carrierGraceTimer);\n          sdr.carrierGraceTimer = null;\n          disconnectSocket();\n          setStatus('Reconnecting', false);\n          setMessage('Receiver dropped almost immediately. Retrying the same receiver once before moving on…', true);\n          window.setTimeout(() => connectSdr(retryIndex), 700);\n        } else {\n          sdr.shortDisconnectRetries = 0;\n          failCurrentReceiver('The public receiver disconnected. Trying the next ranked receiver…');\n        }\n      }\n    };`;
