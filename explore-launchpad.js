(() => {
  ...
  function renderReady(payload) {
    ...
    showChoice(payload.current, 'YOUR RECEIVER');
  ...
  async function loadLaunchpad() {
    ...
    const listeningLocation = storedListeningLocation();
    const url = new URL(RECOMMENDATION_FEED, location.origin);
    if (previousId) url.searchParams.set('previousId', previousId);
    if (listeningLocation) {
      url.searchParams.set('lat', String(listeningLocation.lat));
      url.searchParams.set('lon', String(listeningLocation.lon));
    }
