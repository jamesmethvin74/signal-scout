...
async function recordProbe(env, result) {
  ...
  `).bind(
    ...
    result.sndSuccess ? 1 : 0, result.wfSuccess ? 1 : 0, connectMs, rate, healthScore,
    trusted ? 1 : 0, JSON.stringify(history), result.receiverId
  ).run();
}

async function trustedRows(env) {
  await ensureSchema(env);
  const now = Date.now();
  const result = await db(env).prepare(`
...
export async function handleExploreApi(request, env) {
  ...
  const receivers = await trustedRows(env);
  const features = receivers.map((receiver) => ({
    type: 'Feature',
