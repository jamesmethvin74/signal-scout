(() => {
  ...
  function viewport() {
    ...
    return { left: center - DEFAULT_VIEW_SPAN_KHZ / 2, right: center + DEFAULT_VIEW_SPAN_KHZ / 2 };
  }
  ...
  function drawReadableScale(left, right, width, scaleY, bandH) {
    ...
    for (let i = 0; i <= 8; i += 1) {
      ...
      ctx.fillText((frequency / 1000).toFixed(4), Math.max(3, Math.min(width - 3, x)), scaleTop + 4);
    }
  }
  ...
  function draw() {
    ...
    for (const band of visible) {
      if (band.kind === 'cb') drawCbChannels(left, right, span, width, scaleY, bandH);
      else drawBandBar(band, left, right, span, width, scaleY, bandH);
    }
