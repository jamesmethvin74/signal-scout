(() => {
  ...
  // IDENTIFY tolerates a listener being slightly off the nominal carrier.
  // Broadcast channels get a wider window than fixed utility/service signals.
  ...
  function exactMatch(kHz, receiver, now = new Date(), extraEntries = []) {
    const candidates = mergeEntries(extraEntries)
      ...
      // This prevents a powerful adjacent channel from stealing a widened match.
      .sort((a, b) =>
