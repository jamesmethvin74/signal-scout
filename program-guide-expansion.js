(() => {
  ...
  function render(slot, data) {
    ...
    slot.dataset.programGuideExpanded = 'wwcr';
  }

  async function upgrade(card) {
    const station = stationFrom(card);
    if (!/^WWCR$/i.test(station) || inflight.has(card)) return;
    const slot = card.querySelector('[data-program-guide]');
    if (!slot || slot.dataset.programGuideExpanded === 'wwcr' || slot.classList.contains('is-verified')) return;
    const frequency = frequencyFrom(card);
    if (!Number.isFinite(frequency)) return;

    inflight.add(card);
    const at = targetDate(card);
    const key = `${Math.round(frequency)}|${Math.floor(at.getTime()/60000)}`;
